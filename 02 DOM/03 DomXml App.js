...
function initialize(idTheTable, idXMLData) {
  ...
  if (theTBODY.length > 0) oTheTable.removeChild(theTBODY[0]);
...
function highlightVegetarian(idTable, bShowVeg) {
  ...
  for (i = 0; i < aTRs.length; i++) {
    if (aTRs[i].getAttribute("data-veg")) {
      if (bShowVeg) aTRs[i].style.backgroundColor = "lightGreen";
      else aTRs[i].style.backgroundColor = "";
    }
  }
}
...
function getParentTag(oNode, sParentType) {
  ...
  while (oParent) {
    if (oParent.nodeName == sParentType) return oParent;
